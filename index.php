...
if (strpos($_msg, 'สอนว่า') !== false) {
  if (strpos($_msg, 'สอนว่า') !== false) {
    ...
    $arrPostData['messages'][0]['text'] = 'ขอบคุณที่สอนค่ะ';
  }
}else{
  if($isData >0){
   foreach($data as $rec){
    ...
    $arrPostData['replyToken'] = $arrJson['events'][0]['replyToken'];
    $arrPostData['messages'][0]['type'] = "text";
    $arrPostData['messages'][0]['text'] = 'ผมยังไม่เข้าใจคำถามของคุณครับ\n คุณสามารถสอนผมให้ฉลาดได้เพียงพิมพ์: สอนว่า[คำถาม|คำตอบ]';
  }
}
